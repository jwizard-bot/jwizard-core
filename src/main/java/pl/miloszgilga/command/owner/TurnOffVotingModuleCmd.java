 ...
 *
 * File name: TurnOffVotingModuleCmd.java
...
import static pl.miloszgilga.exception.ModuleException.VotingModuleIsAlreadyDisabledException;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@Slf4j
@JDAInjectableCommandLazyService
public class TurnOffVotingModuleCmd extends AbstractOwnerCommand {
    ...
    TurnOffVotingModuleCmd(
        ...
        super(BotCommand.TURN_OFF_VOTING_MODULE, config, embedBuilder, handler, cacheableCommandStateDao);
    ...
    protected void doExecuteOwnerCommand(CommandEventWrapper event) {
        if (!handler.getPossibleRemoteModuleProperty(RemoteModuleProperty.R_VOTING_MODULE_ENABLED, event.getGuild())) {
            throw new VotingModuleIsAlreadyDisabledException(config, event);
        }
        final CacheableModuleData data = CacheableModuleData.builder()
            ...
            .moduleRemoteSetter(modules -> modules.setVotingModuleEnabled(false))
            .build();
