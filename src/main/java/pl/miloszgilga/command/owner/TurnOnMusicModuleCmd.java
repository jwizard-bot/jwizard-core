 ...
 *
 * File name: TurnOnMusicModuleCmd.java
...
import static pl.miloszgilga.exception.ModuleException.MusicModuleIsAlreadyRunningException;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@Slf4j
@JDAInjectableCommandLazyService
public class TurnOnMusicModuleCmd extends AbstractOwnerCommand {
    ...
    protected void doExecuteOwnerCommand(CommandEventWrapper event) {
        if (handler.getPossibleRemoteModuleProperty(RemoteModuleProperty.R_MUSIC_MODULE_ENABLED, event.getGuild())) {
            throw new MusicModuleIsAlreadyRunningException(config, event);
        }
        final CacheableModuleData data = CacheableModuleData.builder()
            ...
            .moduleRemoteSetter(modules -> modules.setMusicModuleEnabled(true))
            .build();

        final var updatedSettings = cacheableGuildModulesDao.toggleGuildModule(data);
        modulesRepository.save(updatedSettings);

        final MessageEmbed messageEmbed = embedBuilder.createMessage(ResLocaleSet.GUILD_MUSIC_MODULE_ENABLED_MESS, Map.of(
            "disableMusicModuleCmd", BotCommand.TURN_OFF_MUSIC_MODULE.parseWithPrefix(config)
        ), event.getGuild());
        JDALog.info(log, event, "Music module for selected guild '%s' was successfully enabled", event.getGuildName());
